...
export function TreatmentEditor() {
  ...
  const handleSave = async () => {
    try {
      await saveTo(`/appointments/${id}/services`, {
        method: "PUT",
        allowEmpty: true,
      });
      navigate("/doctor/treatments");
    } catch {
      // saveTo already sets error
    }
  };
    ...
    <div className="mx-auto max-w-6xl p-4 pb-32 md:p-6 md:pb-6">
      ...
      <div className="mb-8 rounded-xl bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-bold text-slate-900">
          {t("treatment.addServices") || "Qo'shimcha xizmatlar qo'shish"}
        </h1>
        <p className="mt-2 text-sm text-slate-600">
          {t("treatment.addMoreSubtitle") ||
            "Mavjud davolashga yangi xizmatlar qo'shing"}
        </p>
        ...
        {selectedTeeth.length > 0 && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg text-center">
            <p className="font-medium text-slate-800">
              {t("treatment.selectedTeeth") || "Tanlangan tishlar"}:{" "}
              {selectedTeeth.join(", ")}
            </p>
          </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-4 shadow-lg md:static md:shadow-none">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            {saving ? "Saqlanmoqda..." : t("common.add") || "Qo'shish"}
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-900">
                {t("patients.editPatient") || "Bemorni tahrirlash"}
              </h2>
              <button
                type="button"
